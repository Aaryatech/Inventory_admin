...
<c:choose>
<c:when test="${grnType==0}">
<c:set var="type" value="Regular"></c:set>
</c:when>
<c:otherwise>
<c:set var="type" value="Replace"></c:set>
</c:otherwise>
</c:choose>
<div align="center"> <h5>${type} </h5></div>
		...
		<tbody>
			...
			<c:forEach items="${staticlist}" var="staticlist" varStatus="count">
												<tr>
													...
													<td width="100" style="text-align:right"><c:out value="${staticlist.qty}" /></td>  
